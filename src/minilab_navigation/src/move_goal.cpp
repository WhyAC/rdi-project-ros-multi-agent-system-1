...
int main(int argc, char **argv)
{
	...
	goal_r = speed_linear(current_pose.x, current_pose.y, goal_posex, goal_posey)/0.5;
	int gr = round(goal_r);
	...
	while((ros::ok()) && (abs(current_r-goal_r))<=0.0)
	{
		current_r = speed_linear(current_pose.x, current_pose.y, goal_posex, goal_posey)/0.5;
		int cr = round(current_r);
		mx = speed_linear(current_pose.x, current_pose.y, goal_posex, goal_posey);
		mz = speed_angular(current_pose.x, current_pose.y, goal_posex, goal_posey, current_pose.theta);	
