...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_odometryAgent2 = n.subscribe("/minilab2/odom", 2, odomCallbackagent2);
    ros::Publisher movement_pubAgent2 = n.advertise<geometry_msgs::Twist>("/minilab2/cmd_vel",2);
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2dAgent2 = n.advertise<geometry_msgs::Pose2D>("/minilab2/pose2d", 2);
    
    ros::Rate rate(10); //the larger the value, the "smoother" , try value of 1 to see "jerk" movement
    
    ros::Subscriber sub_odometry = n.subscribe("/minilab1/odom", 2, odomCallbackAgent1);
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2d = n.advertise<geometry_msgs::Pose2D>("/minilab1/pose2d", 2);
	...
	while (ros::ok() && ((t_last.toSec()) > (time_sw)) )
	{
		...
		// calculate e3
		e_3 = e_4*px_agent2 - e_4*px_agent1; // x_2 - x_2d; // position d'agent2 - position du agent1
