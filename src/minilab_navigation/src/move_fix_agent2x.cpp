...
int main(int argc, char **argv)
{
    ...
    double alpha_1=1,beta_1=1;
    double alpha_2=1,beta_2=1;
    ...
    pub_pose2dAgent2 = n.advertise<geometry_msgs::Pose2D>("/minilab2/pose2d", 10);
    
    ros::Rate rate(100); //the larger the value, the "smoother" , try value of 1 to see "jerk" movement
    
    ros::Subscriber sub_odometry = n.subscribe("/minilab1/odom", 10, odomCallbackAgent1);
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2d = n.advertise<geometry_msgs::Pose2D>("/minilab1/pose2d", 10);
    ...
    while(ros::ok())// && ((t_last.toSec()) <= (time_sw)) )
    {
		...
		// calculate e1
		e_1 = px_agent2 - px_agent1; // x_2 - x_2d; // position d'agent2 - position du agent1
				
		// calculate e2
		e_2 = vx_agent2 - vx_agent1; //vxa - vxl; // vitesse d'agent2 - vitesse du agent1
	...
	while (ros::ok() && ((t_last.toSec()) > (time_sw)) )
	{
		...
		// calculate e3
		e_3 = vx_agent2*px_agent2 - vx_agent1*px_agent1; // x_2 - x_2d; // position d'agent2 - position du agent1
				
		// calculate e4
		e_4 = vx_agent2 - vx_agent1; //vxa - vxl; // vitesse d'agent2 - vitesse du agent1
		
		//calculate zeta_4
		zeta_4 = e_4*vx_agent1;
		...
		// calculating u_2
		u_2=(0*(e_4*u_1d)/px_agent1)+vx_agent1+(phi_2);///px_agent1);
		/*if (delta_t.toSec() > 1.0)
		{		
			u_2_integral = u_2 * 0;
		}
