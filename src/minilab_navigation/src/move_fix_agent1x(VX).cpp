...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "move_fix_agent1x");
    ros::NodeHandle n;
    
    ros::Subscriber sub_odometryAgent1 = n.subscribe("/agent1/odom", 2, odomCallbackagent1);
    ros::Publisher movement_pubAgent1 = n.advertise<geometry_msgs::Twist>("/agent1/cmd_vel",2);
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2dAgent1 = n.advertise<geometry_msgs::Pose2D>("/agent1/pose2d", 2);
    
    ros::Rate rate(10); //the larger the value, the "smoother" , try value of 1 to see "jerk" movement
    
    ros::Subscriber sub_odometry = n.subscribe("/leader/odom", 2, odomCallbackLeader);
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2d = n.advertise<geometry_msgs::Pose2D>("/leader/pose2d", 2);
    ...
    while(ros::ok() && ((t_last.toSec()) <= (time_sw)) )
    {
		...
		// calculate e1
		e_1 = vx_agent1 - vx_leader; //e_1 = px_agent1 - px_leader; // position d'agent1 - position du leader
				
		// calculate e2
		e_2 = u_1-u_1d; //vx_agent1 - vx_leader; // vitesse d'agent1 - vitesse du leader
		...
		std::cout << "U1 : " << u_1;
		std::cout << std::endl;
	    std::cout << "Velo 0f Agent : " << vx_agent1;
		std::cout << std::endl;
	...
	while (ros::ok() && ((t_last.toSec()) > (time_sw)) )
	{
		...
		// calculate e3
		e_3 = vx_agent1*e_4 - vx_leader*e_4;//vx_agent1*px_agent1 - vx_leader*px_leader; // x_2 - x_2d; // position d'agent1 - position du leader
				
		// calculate e4
		e_4 = u_2-u_2d;//vx_agent1 - vx_leader; //vxa - vxl; // vitesse d'agent1 - vitesse du leader
		
		//calculate zeta_4
		zeta_4 = e_4 * px_leader;
		...
		// calculating u_2
		u_2=u_2d-((e_4*u_1d)/px_leader)+(phi_2/px_leader);
		/*if (delta_t.toSec() > 1.0)
		{		
			u_2_integral = u_2 * 0;
		}
