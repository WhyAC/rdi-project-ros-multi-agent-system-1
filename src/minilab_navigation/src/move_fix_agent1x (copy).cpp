...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_odometryAgent1 = n.subscribe("/minilab1/odom", 2, odomCallbackagent1);
    ros::Publisher movement_pubAgent1 = n.advertise<geometry_msgs::Twist>("/minilab1/cmd_vel",2);
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2dAgent1 = n.advertise<geometry_msgs::Pose2D>("/minilab1/pose2d", 2);
    
    ros::Rate rate(10); //the larger the value, the "smoother" , try value of 1 to see "jerk" movement
    
    ros::Subscriber sub_odometry = n.subscribe("/minilab3/odom", 2, odomCallbackLeader);
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2d = n.advertise<geometry_msgs::Pose2D>("/minilab3/pose2d", 2);
	...
	while (ros::ok() && ((t_last.toSec()) > (time_sw)) )
	{
		...
		// calculate e3
		e_3 = e_4*px_agent1 - e_4*px_leader; // x_2 - x_2d; // position d'agent1 - position du leader
				
		// calculate e4
		e_4 = vx_agent1 - vx_leader; //vxa - vxl; // vitesse d'agent1 - vitesse du leader
