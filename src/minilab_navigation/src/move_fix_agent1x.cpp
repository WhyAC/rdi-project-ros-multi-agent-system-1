...
int main(int argc, char **argv)
{
    ...
    double alpha_1=2,beta_1=2;
    double alpha_2=2,beta_2=2;
    ...
    pub_pose2dAgent1 = n.advertise<geometry_msgs::Pose2D>("/minilab1/pose2d", 10);
    ...
    //for sensors the value after , should be higher to get a more accurate result (queued)
    pub_pose2d = n.advertise<geometry_msgs::Pose2D>("/minilab3/pose2d", 10);
	...
	while (ros::ok() && ((t_last.toSec()) > (time_sw)) )
	{
		...
		// calculate e3
		e_3 = vx_agent1*px_agent1 - vx_leader*px_leader; // x_2 - x_2d; // position d'agent1 - position du leader
				
		// calculate e4
		e_4 = vx_agent1 - vx_leader; //vxa - vxl; // vitesse d'agent1 - vitesse du leader
		
		//calculate zeta_4
		zeta_4 = e_4*vx_leader;
		...
		// calculating u_2
		u_2=(0*(e_4*u_1d)/px_leader)+vx_leader+(phi_2);///px_leader);
		/*if (delta_t.toSec() > 1.0)
		{		
			u_2_integral = u_2 * 0;
		}
